shortCircuit = "(function() { if (/internet explorer/i.test(window.navigator.userAgent) || /MSIE/i.test(window.navigator.userAgent)) {var dummy = function() {return {restrict: 'E', replace: 'true', template: '<p>Not supported for this browser</p>'};};angular.module('bcQrReader', []).directive('bcQrReader', [dummy]);return;}"
